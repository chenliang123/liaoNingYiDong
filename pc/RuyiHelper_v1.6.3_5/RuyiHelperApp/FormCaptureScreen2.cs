...
namespace RueHelper
{
    public partial class FormCaptureScreen2 : Form
    {
        ...
        public Bitmap CaptureImage(Image fromImage, int offsetX, int offsetY, int width, int height)
        {
            //创建新图位图
            Bitmap bitmap = new Bitmap(width, height);
            //创建作图区域
...
//?,1              
        ...
        private void pictureBox1_MouseClick(object sender, MouseEventArgs e)
        {
            ...
            m_pointLast = p;
            Log.Info("click. " + p.ToString());
        }

        private void pictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            Point pt2 = MousePosition;
            if (e.Button != MouseButtons.Right && e.Button != MouseButtons.Left)
            {

                Log.Info("move0. " + pt2.ToString());
                return;
            }
            if (m_pointLast.X == 0 && m_pointLast.Y == 0)
            {
                m_pointLast = MousePosition;
            }

            Log.Info("move1. " + pt2.ToString());
            try
            {
                if (IsPixelFormatIndexed(m_bmp.PixelFormat))
                {
                    ...
                    using (Graphics g = Graphics.FromImage(bmp))
                    {
                        ...
                        //draw.....
                        Color c = ColorTranslator.FromHtml(lineColor);
                        Pen pen = new Pen(c, lineWidth);
                        g.DrawLine(pen, m_pointLast, pt2);
        ...
        private void FormCaptureScreen2_MouseUp(object sender, MouseEventArgs e)
        {
