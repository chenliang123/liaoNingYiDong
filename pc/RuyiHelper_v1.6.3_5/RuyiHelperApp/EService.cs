...
namespace RueHelper
{
    ...
    public class EService : IEService
    {
        ...
        private void Close()
        {
            ...
            if (fSelectTeacher != null)
            {
                fSelectTeacher.Close();
                fSelectTeacher = null;
            }
            if (fCp != null)
            {
        ...
        public string CloseProjective()
        {
            ...
            if (result.Length > 0)
            {
                ...
                //Common.uploadXitiResult(xitiResult);    //屏蔽数据上传，该由前端上传；
            }
        ...
        public int ProjectiveInPPT(string courseid, string classid, string lessonid)
        {
            Global.panelshow = 1;
            int temp = 0;
            ...
            else
            {
                ...
                //fPPTPractise.Close();
                //fPPTPractise = null;
                temp = ProjectiveInPPT(courseid, classid, lessonid);
        ...
        public int JudgeProjectiveInPPT(string courseid, string classid, string lessonid)
        {
            Global.panelshow = 1;
            int temp = 0;
            Log.Info("ProjectiveInPPT() courseid=" + courseid + ", classid=" + classid + ", lessonid=" + lessonid);
            ...
            {
                CloseProjective();
                temp = JudgeProjectiveInPPT(courseid, classid, lessonid);
        ...
        public static void ShowSelectTeacher(Boolean show, int type)
        {
            if (show)
            {
                if (fSelectTeacher == null)
                {
                    fSelectTeacher = new FormSelectTeacher(type);
                }
                else
                {
                    fSelectTeacher.childForm = null;
                    fSelectTeacher = null;
                    fSelectTeacher = new FormSelectTeacher(type);
                }
                fSelectTeacher.Show();
