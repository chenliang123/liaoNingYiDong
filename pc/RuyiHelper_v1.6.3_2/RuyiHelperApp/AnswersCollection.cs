...
namespace RueHelper
{
    public class AnswersCollection
    {
        ...
        [DllImport(DLL_NAME, EntryPoint = "HX_Reset", CallingConvention = CallingConvention.StdCall)]
        internal static extern int HX_Reset();
    ...
    {
        HX_MODE_NONE = 0x00, 	//待机模式
        HX_MODE_SINGLE = 0x01,	//单题模式
        HX_MODE_SINGLE_judge = 0x81, //单题模式判断
        HX_MODE_SINGLE_MUL = 0x41,	//单题模式多选
        HX_MODE_RAISE = 0xC1,	//单题模式抢答（举手）----提交数字
        HX_MODE_RAISE1 = 0x08,  //投票模式（单选答案提交）
        HX_MODE_MULTI = 0x02,	//多题模式
        HX_MODE_PAPER = 0x03		//套卷模式
    }
}